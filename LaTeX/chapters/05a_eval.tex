...
\begin{comment}
    #TODO: go on from this DONE BY ANDREA
\end{comment}
\subsubsection*{Resampling Methods}
To assess how much our evaluation metric might fluctuate due to data sampling, we rely on \textbf{resampling methods}.
...
For instance, in cross-validation, we compute the metric on each splitted fold.
By analyzing how much the metric varies across these folds, we gain insight into the robustness of our results and the degree to which they depend on the particular sample at hand.

However, as we increase the number of repetitions, especially with small datasets, the overlap between samples also increases, which can limit the independence of our estimates.

\subsubsection*{Bootstraping Methods}
An alternative approach is to use \textbf{bootstrapping methods}, where we repeatedly \textbf{resample with replacement} from the original dataset to generate multiple new samples, called bootstrap samples.
Each bootstrap sample is the same size as the original dataset, but because sampling is done with replacement, some data points may appear multiple times or not at all.

This technique allows us to create as many resampled datasets as needed, without being limited by overlap concerns.
By evaluating our metric on each bootstrap sample, we can estimate its variability and construct confidence intervals, often with greater flexibility than traditional resampling methods.
...
For small sample sizes, the distribution of the sample mean follows a Student's t-distribution, which approaches a normal distribution as the sample size increases ($n > 30$), both centered at the true mean.
This statistical framework enables us to construct confidence intervals for the true metric, using the sample mean and its observed variability across samples.
...
Bias and variance are two fundamental sources of error in regression models, and their definitions are closely tied to the mean squared error (MSE), though they can be extended to other metrics as well.
\begin{itemize}
    \item \textbf{Bias}: This is the difference between the optimal MSE (the lowest possible error for your model class) and the true MSE.
...
\end{itemize}
Typically, there is a trade-off between bias and variance: increasing model complexity tends to reduce bias but increase variance, while simpler models have higher bias and lower variance.

\subsection{Evaluating Classification}
When evaluating classification models, we rely on metrics such as accuracy, precision, recall, and F1-score. These metrics are derived from the confusion matrix, which summarizes the counts of true positives, true negatives, false positives, and false negatives.
...
From this table, we can compute a variety of metrics to evaluate the model, for example the \textbf{accuracy}, which is the proportion of correctly classified instances.
...
Typically, we use \textbf{test accuracy} to assess the validity of our conclusions, \textbf{validation accuracy} to tune hyperparameters, and \textbf{training accuracy} to compare against validation accuracy (the \textbf{generalization gap}) to detect overfitting or underfitting.
Training accuracy alone should not be used for model evaluation.

\subsubsection{Imbalance Issues}
Imbalance issues arise when classes are not equally represented or have different importance.

\textbf{Class imbalance} occurs when one class is much more frequent than the other, making high accuracy misleading.
For example, a model may simply predict the majority class and still achieve a high score.
...
Furthermore, the cost of misclassifying one class (\textbf{cost imbalance}) can be much higher than the other (e.g., failing to detect a sick patient is typically worse than a false alarm).
In such cases, it is important to use metrics and error calculations that reflect both class imbalance and cost imbalance, often by weighting errors according to their impact based on the specific application.
...
\subsubsection{Coverage Matrix} %TODO: Choose which to keep and if its right,
For ranking classifiers, we can visualize their predictions using a \textbf{coverage matrix}.
In this plot, the x-axis represents the confidence for the negative class, and the y-axis represents the confidence for the positive class.

Points near the top right corner show strong confidence in predicting positive, while points near the bottom left show strong confidence in predicting negative.
By coloring each point according to the predicted class (after applying a threshold), we get an intuitive visual summary of how well the model separates the two classes.

\subsubsection{Coverage Matrix}
For ranking classifiers, a \textbf{coverage matrix} visualizes all possible pairs of positive (rows) and negative (columns) examples.
